...
-- Trend sussidi per dominio CEPA: primo/ultimo anno, CAGR, classificazione
...
primo_ultimo AS (
    SELECT
        cep_class,
        FIRST_VALUE(sussidi_mln_eur) OVER w AS primo_val,
        FIRST_VALUE(year) OVER w AS anno_primo,
        LAST_VALUE(sussidi_mln_eur) OVER w AS ultimo_val,
